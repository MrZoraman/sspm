...
class Uv(Dependency):
    ...
    def __artifact_dll(self):
        return f"{self.lib_build_dir()}/libuv-1.44.2/build/Release/{ARTIFACT_DLL_NAME}"
    
    def __artifact_lib(self):
        return f"{self.lib_build_dir()}/libuv-1.44.2/build/Release/{ARTIFACT_LIB_NAME}"
    ...
    def build(self):
        ...
        build_dir = f"{self.lib_build_dir()}/libuv-1.44.2"
        os.system(
            "cmake "
            f"-S {build_dir} "
            f"-B {build_dir}/build "
    ...
    def setup_cmake(self):
        with open(self.cmake_file(), 'w') as file:
            file.write(f"set(UV_INCLUDE_DIR {self.include_dir()} PARENT_SCOPE)\n")
            file.write(f"set(UV_LIB {self.static_lib_file(ARTIFACT_LIB_NAME)} PARENT_SCOPE)\n")
